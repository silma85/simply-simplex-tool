...
public class OriginalProblemFrame extends JFrame implements ActionListener, ChangeListener {
	...
	public OriginalProblemFrame(final String title) throws HeadlessException {
		...
		this.setIconImage(ResourceLoader.loadImageIcon("logo.png").getImage());
	...
	public void actionPerformed(final ActionEvent e) {
		...
		if (e.getActionCommand().equals(startSimplexButton.getActionCommand())) {
			checkNegativeKnowns();
			final boolean needed = isArtificialNeeded();
			doArtificialRequest(needed);
	...
	private void addTopInputFields(final JPanel typePanel, final JPanel canonFormPanel, final JPanel welcomePanel) {
		...
		// Per retrocompatibilita' totale.
		// typePanel.setLayout(new GridLayout(3, 2));
		// typePanel.add(typeLabel);
		// typePanel.add(problemTypeSpinner);
		// typePanel.add(variablesLabel);
		// typePanel.add(varSpinner);
		// typePanel.add(constraintsLabel);
		// typePanel.add(constraintSpinner);
	 ...
	 * artificiale.
	 */
	private void doArtificialRequest(final boolean needed) {
	...
	private void initMenus() {
		...
		exportAmplFileMenu.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent e) {
				expo.generateAMPL();
			}
		...
		infoHelpMenu.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(final ActionEvent e) {
				if (!helpDialog.isVisible()) {
					...
					try {
						helpPane.setPage(ResourceLoader.loadURL("readme.html"));
